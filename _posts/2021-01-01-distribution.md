...
# 초함수(distribution)
 ...
 초함수들의 집합은 $D\star$ 라고 쓴다. 별표가 붙은 것에서 눈치챌 수 있듯이, $D$의 dual space라고 생각할 수 있는데, 조건이 좀 더 붙는다. $F \in D^\star$ 는 continuous linear functional on $D$로, $F(\phi_i) \rightarrow F(\phi)$ whenever $\phi_i \rightarrow \phi$ in $D$를 만족해야 한다.
